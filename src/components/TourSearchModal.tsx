                ...
                <IonList>
                    {
                        renderPlaces?.map((place: { mainpic: string | undefined; about: any | undefined, name: React.ReactNode; }) => {
                            return <SearchResultItem place={place}></SearchResultItem>
                        })
                    }
...
const SearchResultItem: React.FC<{place:any}> = ({place}) => {

    const [selectedplace, setselectedplace] = useState<any>()

    return (
        <IonItem button onClick={() => { setselectedplace(place) }}>
            <IonGrid>
                <IonRow>
                    <IonCol>
                        <IonThumbnail>
           ...
           {/* {selectedplace&& <TourInfo isOpen={!!selectedplace} placedata={selectedplace} onDidDismiss={() => setselectedplace(undefined)} />} */}
        </IonItem>
    )
}